...
# 1)movies directed by peter jackson
# Peter_Jackson_movies=[m.get("title") for m in movies if m.get("director")=="Peter Jackson"]
# print(Peter_Jackson_movies)
# 2)total number of movies
# print(len(movies))
#3)all movies name
# movies_names=[m.get("title") for m in movies]
# print(movies_names)
#4)run time greater than 150
# movie_with_run_time=[m.get("title") for m in movies if int(m.get("runtime"))>150]
# print(movie_with_run_time)
# 5)most play time movie
# movie=max(movies,key=lambda m: int(m.get("runtime")))
# 6)min play time movie
# movie=min(movies,key=lambda m:int(m.get("runtime")))
# 7)print movie with genere comedy and music
# movie=[m.get("title") for m in movies if "Comedy" in m.get("genres") and "Drama" in m.get("genres")]
# 8)print movies with more than 2 geners
# movie=[m.get("title") for m in movies if len(m.get("genres"))>2]
# 9)print movie in a series
# movie=[m.get("title") for m in movies if m.get("title").startswith("The Lord of the Rings")]
# 10)yearwise movies
# movie={}
# year=[]
# for m in movies:
#     year.append(int(m.get("year")))
# year.sort()
# for y in year:
#     if y in movie:
#         movie[y]+=1
...
#         movie[year]=1
# print(movie)
# most film released year
# value_key=[(v,k) for k,v in movie.items()]
# print(max(value_key))
jc={}
for m in movies:
    geners=m.get("genres")
    for g in geners:
        if g in jc:
            ...
            jc[g]=1 
print(jc)
value_key=[(v,k) for k,v in jc.items()]
print(max(value_key))
print(min(value_key))
